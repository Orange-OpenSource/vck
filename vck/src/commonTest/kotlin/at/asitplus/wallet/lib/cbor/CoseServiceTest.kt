...
val CoseServiceTest by matrixSuite {
    ...
    fixture {
        object {
            ...
            val signCoseNothing = SignCose<Nothing>(signKeyMaterial)
            val signCoseMso = SignCose<MobileSecurityObject>(signKeyMaterial)
            val signCoseDetached = SignCoseDetached<ByteArray>(signKeyMaterial)
            val randomPayload = Random.nextBytes(32)
            val signCoseKey = signKeyMaterial.publicKey.toCoseKey().getOrThrow()
            ...
            val macCose = MacCose<ByteArray>(macCoseKey)
            val macCoseMso = MacCose<MobileSecurityObject>(macCoseKey)
            val macCoseNothing = MacCose<Nothing>(macCoseKey)
            val macCoseDetached = MacCoseDetached<ByteArray>(macCoseKey)
        }
    } - {
        ...
        test("signed object with random bytes can be verified") {
            ...
            signed.payload shouldBe it.randomPayload
            signed.signature.shouldNotBeNull()
        ...
        test("maced object with random bytes can be verified") {
            ...
            maced.payload shouldBe it.randomPayload
            maced.tag.shouldNotBeNull()
        ...
        test("maced object with MSO payload can be verified") {
            ...
            VerifyCoseMacWithKey<MobileSecurityObject>()(
                parsed,
                it.macCoseKey,
                byteArrayOf(),
                null
        ...
        test("signed object with random bytes, transported detached, can be verified") {
            ...
            with(VerifyCoseSignatureWithKey<ByteArray>()) {
                invoke(parsed, it.signCoseKey, byteArrayOf(), it.randomPayload).isSuccess shouldBe true
                invoke(parsed, it.signCoseKey, byteArrayOf(), it.randomPayload + byteArrayOf(0)).isSuccess shouldBe false
                invoke(parsed, it.signCoseKey, byteArrayOf(), null).isSuccess shouldBe false
            }
        }
        ...
        test("maced object with random bytes, transported detached, can be verified") {
            ...
            with(VerifyCoseMacWithKey<ByteArray>()) {
                invoke(parsed, it.macCoseKey, byteArrayOf(), it.randomPayload).isSuccess shouldBe true
                invoke(parsed, it.macCoseKey, byteArrayOf(), it.randomPayload + byteArrayOf(0)).isSuccess shouldBe false
                invoke(parsed, it.macCoseKey, byteArrayOf(), null).isSuccess shouldBe false
            }
        }
